...
try:
    # แบบที่ 1: พยายามดึงรหัสผ่านจากระบบ Secrets ของเว็บ Streamlit Cloud
    creds_dict = dict(st.secrets["gcp_service_account"])
    creds = Credentials.from_service_account_info(creds_dict, scopes=scope)
    client = gspread.authorize(creds)
except Exception:
    # แบบที่ 2: ถ้าหาไม่เจอ (แปลว่ากำลังรันทดสอบในคอมตัวเอง) ให้ดึงจากไฟล์แทน
    try:
        creds = Credentials.from_service_account_file("credentials.json", scopes=scope)
        client = gspread.authorize(creds)
    except FileNotFoundError:
        st.error("🚨 ระบบตรวจไม่พบไฟล์รหัสผ่าน กรุณาตั้งค่า Secrets หรือวางไฟล์ credentials.json ครับ")
        st.stop()
