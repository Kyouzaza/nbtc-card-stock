...
def render_operator_legend():
    ...
    for operator in OPERATORS:
        ...
        chips.append(
            f"""
            <span class="operator-chip" style="--operator-bg:{colors['bg']};--operator-border:{colors['border']};--operator-text:{colors['text']};">
                {operator}
            </span>
            """
        )
...
def render_stock_summary(df, columns):
    ...
    operator_summary = (
        ...
        .agg(
            จำนวน=(columns["Operator"], "size"),
            มูลค่ารวม=(columns["Price"], "sum"),
        )
        .reset_index()
    )
    ...
    for index, operator in enumerate(OPERATORS):
        ...
        count = int(operator_rows["จำนวน"].sum()) if not operator_rows.empty else 0
        value = float(operator_rows["มูลค่ารวม"].sum()) if not operator_rows.empty else 0
...
def render_raw_table(df, columns):
    ...
    if columns.get("Operator"):
        ...
        st.dataframe(df.drop(columns=["sheet_row"], errors="ignore"), use_container_width=True, hide_index=True)
