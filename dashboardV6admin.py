...
def format_used_display(status):
    ...
    # Use larger, colored emoji + short Thai labels for clearer visibility
...
def render_add_card_form(sheet):
    # แสดงชิปแสดงหมายเลข ID ถัดไปต่อแต่ละ Operator บนหน้าเพิ่มบัตร
    try:
        id_col = columns.get("ID") if 'columns' in globals() else None
        chips = []
        if id_col and not df.empty:
            for operator in OPERATORS:
                op_rows = df[df[columns["Operator"]].astype(str).str.upper() == operator]
                ids = op_rows[id_col].astype(str).fillna("")
                extracted = ids.str.extract(r"(\d+)", expand=False)
                nums = pd.to_numeric(extracted, errors="coerce")
                if nums.dropna().empty:
                    next_id = "1"
                else:
                    next_id = str(int(nums.max()) + 1)
                colors = operator_style(operator)
                chips.append(
            ...
            # ถ้าไม่มีคอลัมน์ ID ให้แสดง legend ปกติ
            for operator in OPERATORS:
                colors = operator_style(operator)
                chips.append(
...
def render_withdraw(df, sheet, columns):
    ...
    # Filter by one or more selected operators
    if operator_filter:
...
def render_raw_table(df, columns):
    ...
    # Allow filtering by operator in the raw data view
...
# --- Temporary test users (username: password, role)
# admin can do everything; tester is a normal user
TEST_USERS = {
...
# Sidebar login form (temporary credentials)
st.sidebar.header("เข้าสู่ระบบ (ทดสอบ)")
...
def safe_rerun():
    ...
    try:
        ...
        pass
    try:
        # some versions may expose different helper
        if hasattr(st, "rerun"):
            st.rerun()
            return
    except Exception:
